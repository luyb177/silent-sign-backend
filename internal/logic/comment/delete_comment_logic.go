...
func (l *DeleteCommentLogic) DeleteComment(req *types.DeleteCommentReq) (resp *types.Response, err error) {
	...
	err = l.svcCtx.Repos.Transaction(func(tx *gorm.DB) error {
		...
		return l.afterDelete(comment, tx)
	})
	if err != nil {
...
func (l *DeleteCommentLogic) afterDelete(comment *commentRepo.Comment, tx *gorm.DB) error {
	switch comment.TargetType {
	case constvar.TargetTypeMoment:
		return l.decrMomentComment(comment.TargetID, tx)
	}
	return nil
}

// decrMomentComment 原子减少动态评论数并刷新热度
func (l *DeleteCommentLogic) decrMomentComment(momentID uint64, tx *gorm.DB) error {
